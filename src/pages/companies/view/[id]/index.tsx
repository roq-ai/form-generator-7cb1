...
import { deleteFormById } from 'apiSdk/forms';
...
function CompanyViewPage() {
  ...
  const formHandleDelete = async (id: string) => {
    setDeleteError(null);
    try {
              ...
              {hasAccess('form', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Stack spacing={2} mb={8}>
                  <Flex justifyContent="space-between">
                    ...
                    <NextLink passHref href={`/forms/create?company_id=${data?.id}`}>
                  ...
                  <TableContainer mb={4}>
                    <Table variant="simple">
                      ...
                      <Tbody>
                        {data?.form?.map((record) => (
                          <Tr cursor="pointer" onClick={() => router.push(`/forms/view/${record.id}`)} key={record.id}>
                            <Td>{record.name}</Td>
                            <Td>
